...
public final class Conversation: @unchecked Sendable {
    ...
    public var onResponseDone: ((Response) -> Void)?
    public var onSessionUpdated: (() throws -> Void)?
    public var onTranscriptReceived: ((String) -> Void)?
    ...
    public required init(configuring sessionUpdateCallback: SessionUpdateCallback? = nil) {
        ...
        task = Task.detached { [weak self] in
            ...
            do {
                for try await event in self.client.events {
                    ...
                    catch { throw ConversationError.unhandled("Unhandled error in event handler: \(error)") }

                    guard !Task.isCancelled else { break }
                }
            } catch {
                throw ConversationError.unhandled("Unhandled error in conversation task: \(error)")
            }
        }
    }
    ...
    public func disconnect() {
        ...
        do {
            try send(event: .cancelResponse())
            try send(event: .outputAudioBufferClear())
        } catch {}
        */
        client.disconnect()
        errorStream.finish()
    ...
    public func connect(ephemeralKey: String, model: Model = .gptRealtime) async throws {
        do {
            try await connect(
                using: .webRTCConnectionRequest(ephemeralKey: ephemeralKey, model: model)
            )
    ...
    public func updateSession(withChanges callback: (inout Session) throws -> Void) throws {
        guard var session
        else { throw ConversationError.sessionNotFound }
